...
public class NakamaConnection : MonoBehaviour
{
    ...
    public async void StartNakamaConnection()
    {
        ...
        // 5. Check if the player exists
        bool hasPlayer = await CheckIfPlayerExists(session.UserId);
        if (!hasPlayer)
        {
            int maxRetries = 5; 
            ...
            for (int attempt = 1; attempt <= maxRetries; attempt++)
            {
                try
                {
                    ...
                    Debug.Log($"Attempt {attempt}: Calling create-player RPC with payload: {payload}");
                    
                    var response = await client.RpcAsync(session, "tx/game/create-player", payload);
    ...
    private async Task<bool> CheckIfPlayerExists(string userId)
    {
        try
        {
            string payload = $"{{\"Nickname\": \"{userId}\"}}";
            var response = await client.RpcAsync(session, "query/game/player-nickname", payload);
            Debug.Log("Player found: " + response.Payload);
            return true;
        }
        catch (Exception ex)
        {
